...
async function call_sncf(body, my_proxy) {
    console.log("MY DAAAAAAAAAAAAAATE", body.date)
    let mega_response = [] 
    const date_list = ["T04:00:00.000Z", "T08:00:00.000Z", "T10:00:00.000Z", "T12:00:00.000Z", "T14:00:00.000Z", "T16:00:00.000Z", "T18:00:00.000Z", "T20:00:00.000Z"]
    for (let date_time of date_list) {
        await delay(500);
        const response = await axios.post(
            'https://www.sncf-connect.com/bff/api/v1/itineraries', {
                'schedule': {
                    'outward': {
                        'date': body.date+date_time,
                        'arrivalAt': false
                    }
                },
                'mainJourney': {
            ...
            {
                httpsAgent: new HttpsProxyAgent.HttpsProxyAgent(my_proxy)            
            }
        );
        for(let proposal of response.data.longDistance.proposals["proposals"]) {
            // console.log(proposal)
            if (proposal["bestPriceLabel"] == "0 €") {
                mega_response.push({price: proposal["bestPriceLabel"],
                                    time: proposal.departure["timeLabel"]})

            }
        }
        // mega_response.push(response.data.longDistance.proposals["proposals"])
    }

    console.log(mega_response)
    // console.log("Price:", response.data.longDistance.proposals["bestPriceLabel"])
    // console.log("Price:", response.data.longDistance.proposals)

    return mega_response;
}

async function get_price(body) {
    const response_final = proxy.proxyGenerator.then(async my_proxy => {
        const response_sncf = await call_sncf(body, my_proxy)
        // const response_sncf = await test_axios_proxy(my_proxy, body)
        return response_sncf
    })
