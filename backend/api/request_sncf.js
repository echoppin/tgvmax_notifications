...
async function call_sncf(body, my_proxy) {
    console.log("MY PROXYYYYYYYYYYYYYYYYYYY", my_proxy)
    ...
    console.log("RESPONSSSSSSSSSSSSE", response)

    return response;
}

async function test_axios_proxy(my_proxy, body) {
    ...
    const response = await request(options, function(error, response, html) {
        if (!error && response.statusCode == 200) {
          console.log(response);
        } else {
          console.log("Error scraping site, please try again", response);
        }
      })
    console.log("RESPONSEEEEEEEE", response)


//     const response = await axios.get('http://httpbin.org/get?answer=42', {
//   proxy: {
//     host: my_proxy.ip,
//     port: my_proxy.host
//   }
